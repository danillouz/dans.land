...
> [!note]
>
> The lock-service tradeoff applies to coordination across multiple instances.
> Within one process, an in-memory lock or single-flight mechanism can be enough.

### Probabilistic (early) revalidation

By using a probabilistic decision, fewer requests need to revalidate the cache at the same time:
it basically rolls a die if it should revalidate the cache.
